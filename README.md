...
### Technologies Used
React: Frontend framework for building the user interface. Use ```axios``` library to make HTTP requests to Spring Boot application.
Spring Boot: Backend framework for handling server-side logic and APIs. The React application can communicate with the backend if into the Spring Boot is enabled CORS (Cross-Origin Resource  Sharing).
